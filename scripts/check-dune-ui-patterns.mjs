...
for (const [name, source] of [["add form", addFormSource], ["edit form", editFormSource]]) {
  ...
  assert.match(source, /resetValue\?: number/, `${name}: steppers should accept source-derived reset baselines`)
  assert.match(source, /const resetTarget = typeof resetValue === "number"/, `${name}: steppers should reset to the derived baseline when available`)
  assert.match(source, /group-hover:opacity-100[\s\S]*?aria-label=\{`Reset \$\{label \?\? "value"\} to \$\{resetTarget\}`\}/, `${name}: stepper reset should be contextual and visually quiet`)
  assert.doesNotMatch(source, /h-8 w-8 shrink-0[\s\S]*?↺/, `${name}: stepper reset should not use a persistent square button beside the input`)
}
